<template>
  <view>
    <!-- 1.0 轮播图 -->
    <swiper :indicator-dots="true" autoplay :interval="3000" :duration="1000" circular indicator-color="rgba(255,255,255,.5)"
 ...
 import { addLocalGoods } from '@/utils/goodsStorageTool.js'
  export default {
    ...
    onLoad(options) {
      ...
      // 轮播
      this.getSlidesData(options.goods_id);
      // 获取本地存取地址
    ...
    methods: {
      // 获取轮播数据
      async getSlidesData(goods_id) {
        const result = await this.$axios.get(`api/public/v1/goods/detail?goods_id=${goods_id}`)
      ...
      chooseAddress() {
        wx.chooseAddress({
          success: res => {
            this.addressInfo = `${res.provinceName} ${res.cityName} ${res.countyName}`;
            wx.setStorageSync('address', res);
          },
          fail: err => {
            wx.showModal({
              title: '提示',
              content: '您需要去我的 > 设置里面设置开启授权，获取地址信息',
